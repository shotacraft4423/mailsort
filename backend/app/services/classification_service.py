...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "【priority / urgency（優先度・緊急度）の判定基準】\n"
    "契約解除・クレーム・重大インシデント・重要顧客からの至急対応依頼など、"
    "放置すると営業上のリスクや機会損失に直結する内容は urgent または high "
    "にしてください。期限が迫っている依頼、決裁者・役職者からの直接の連絡、"
    "金額の大きい案件の重要な進捗（受注確定・失注の兆候など）も high 以上を"
    "検討してください。通常の案件紹介・人材紹介・日常的なやり取りは normal、"
    "広告・自動配信・お礼メールなど対応不要なものは low にしてください。\n\n"
    "【重要カテゴリの判定基準】\n"
    "priority/urgency を urgent または high と判定した内容は、案件紹介・"
    "人材紹介など他のカテゴリに加えて『重要』もcategoriesに含めてください"
    "（1件のメールが複数カテゴリに該当するのは正常です。案件紹介であり"
    "同時に重要、ということもあります）。"
)
