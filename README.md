...
#### `.exe`（NSIS）と `.msi` の違い、どちらを配ればいいか

`build_installer.ps1` は両方生成しますが、**配る相手には基本的にどちらか片方（`.exe`）だけで
十分**です。

| | NSIS (`MailSort_<version>_x64-setup.exe`) | MSI (`MailSort_<version>_x64_en-US.msi`) |
|---|---|---|
| 用途 | 個人・少人数への配布向け。世の中の大半のWindowsアプリのインストーラーはこちら | 会社のIT部門がGroup Policy / SCCM等で一括配布する場合向け |
| サイズ・見た目 | 軽量、独自のインストール画面 | Windows標準の「Windows Installer」画面 |
| アンインストール | 「アプリと機能」から通常通り可能 | 同上 |

同僚数人に配る、自分の別PCに入れる、といった用途なら **`.exe`（NSIS版）を渡せば十分**です。
MSIは「社内のIT部門が管理ツールで一斉展開したい」といった特殊な事情がない限り使う必要はありません。

#### 配布時に同梱すべきもの / 相手に必要なもの

**同梱するのはインストーラー1ファイルだけで大丈夫です**（`MailSort_<version>_x64-setup.exe`）。
ソースコード・`.venv`・`node_modules`・Python・Node.js・Rustなど、開発に使ったものは一切
不要です。

配る相手の環境について:
- **Windows 10 / 11** であること（Tauriデスクトップアプリの対応OS）。
- **WebView2ランタイム**: Windows 11には標準搭載、Windows 10でも大半の環境に既に入っています。
  念のため、`tauri.conf.json` で `webviewInstallMode` を `embedBootstrapper` にしてあるので、
  仮に入っていない環境でも、インストーラー自身が導入用の小さなブートストラッパー
  （+1.8MB程度）を内包しており、初回インストール時に自動で導入されます。
- **コード署名はしていません**: 初回起動時にWindows SmartScreenが
  「WindowsによってPCが保護されました」という警告を出すことがあります。これは有料の
  コード署名証明書を使っていない自作アプリでは一般的な挙動で、動作に問題があるわけでは
  ありません。表示されたら「詳細情報」→「実行」で起動できます。社外の不特定多数に配る
  場合は、コード署名証明書の取得を検討してください（個人・小規模配布であれば不要です）。
- **AIプロバイダーの設定**: APIキーなどは同梱されません。相手は初回起動後に自分で
  「設定」タブからAPIキーを入力する必要があります（もしくはAI無効のまま、ローカル簡易
  分類だけの通常メーラーとしても使えます）。
- プラグインを使わないなら、上記の補足にある `plugins/` フォルダの手動配置は不要です。
